...
type ProcessBlurEventOptions = ProcessEventOptions &
    Pick<UseProcessEventOptions, 'onBlur'>

type processFocusEventEventOptions = ProcessEventOptions &
    Pick<UseProcessEventOptions, 'onFocus'>
...
type ProcessPressInEventOptions = ProcessEventOptions &
    Pick<UseProcessEventOptions, 'onPressIn'>

type processPressOutEventEventOptions = ProcessEventOptions &
    Pick<UseProcessEventOptions, 'onPressOut'> & {mobileDevice: boolean}

type ProcessStateChangeOptions = Pick<
...
export const useOnEvent = ({
    ...
    const processPressIn = useCallback(
        (event: GestureResponderEvent) =>
            processPressInEvent(event, {processState, onPressIn}),
    ...
    const processLongPress = useCallback(
        (event: GestureResponderEvent) =>
            processLongPressEvent(event, {processState, onLongPress}),
        [onLongPress, processState]
    )

    const processPressOut = useCallback(
        (event: GestureResponderEvent) =>
            processPressOutEvent(event, {
                processState,
                onPressOut,
                mobileDevice
            }),
    ...
    const processHoverIn = useCallback(
        (event: MouseEvent) =>
            processHoverIntEvent(event, {processState, onHoverIn}),
        [onHoverIn, processState]
    )

    const processHoverOut = useCallback(
        (event: MouseEvent) =>
            processHoverOutEvent(event, {processState, onHoverOut}),
    ...
    const processLayout = useCallback(
        (event: LayoutChangeEvent) =>
            processLayoutEvent(event, {processState, onLayout}),
