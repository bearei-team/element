...
export const formStore = <T extends Store>(): FormStore<T> => {
    let {callback, error, fieldEntities, initialValue, store} =
        createFormContext<T>()

    const getFieldEntities = (signOut = false) =>
        signOut ? fieldEntities : (
    ...
    const getInitialValue = ((name?: NamePath<T>) => {
        ...
        const processValue = (entityName?: keyof T) =>
            entityName &&
            (value = {...value, [entityName]: initialValue[entityName]})
    ...
    const setFieldValue = (
        ...
        const entities = getFieldEntities()
        const processStoreUpdate = (
            onStoreChange?: (changedValue: T, value: T) => void
        ) => {
            store = {...store, ...value}
